...
## 만료 리마인더 (2026-07-19 추가)
- Cron 매일 KST 10:00: 학생·챕터별 **최신** expires_at 기준 D-3/D-2/D-1/당일/D+3 문자
  (연장하면 만료일이 미래로 이동 → 자동 제외). 톤: 재등록 독촉이 아니라 학습 완료 안부.
- 수동/테스트: POST /admin/expiry-reminders/run {dry:true} (teacherGate) — 드라이런으로 대상 미리보기
- 발송 시 관리자 요약 SMS. 상한 100건/회. 실발송·정리 테스트 완료.

## 리마인더 확정판 + 결제 감시 (2026-07-19 완료)
- 문구 5종: 안부→연장 안내→딥링크(https://www.memoryking.kr/?chapter=ID). D-3에 누적 연장(손해 없음) 명시
- 결제-연장 이중 체크 매시 Cron (C1 자동화 감시, 불일치 시 관리자 문자) + CRM 주간 루틴 w_c1 항목
- 상세: ONEPAGE_FEATURES §22.10-c
