...
22.10. [수학 문제은행 — math-bank](#2210-수학-문제은행--math-bank-v241)
...
| ⑤ 회상 | 카드 유형 **자동 감지** (설정 없음): `caption='암기카드'` item 보유 = **문제형**(수학 등) → 앞면에 **문제 발문** + "✏️ 먼저 풀어보세요" / 없으면 **단어형** → 단어만 + 🤔 "뜻을 떠올려 보세요". 터치 → 정답 공개(`revealIn`), 문제형은 **풀이 토글 자동 펼침**. 발음 🔊은 공개 전에도 가능 |
| ⑥ 판정 | 오늘 → 카드가 **왼쪽**으로, 내일 → **오른쪽**으로 회전하며 날아감 + 해당 카운터 **bump** → 다음 카드 **slide-in** (자동). 아는 단어는 **공개 없이 바로 내일**로 보내도 됨 |
...
- **세션 수식 렌더**: MathJax 지연 로더는 `chapterTree`만 감시 → 오버레이는 `sesTypeset()`으로 렌더 3지점(진입·로드완료·공개)에서 명시 typeset. **세션에 콘텐츠 렌더 지점을 추가하면 반드시 같이 호출할 것.**
- 세션은 트리를 **재렌더하지 않는다** → 위 스크롤 앵커 로직이 불필요(오버레이라 리스트가 안 보임).
- **프리페치**: 선택 즉시 + 세션 진행 중 다음 2장을 `ensureItems(sid, true)` → 넘길 때 로딩 없음.
- **세션은 항상 회상 우선** (능동 회상 설정과 무관): 단어만 먼저 → 터치로 공개. **블러가 아니라 완전 가림** — 블러는 암기 이미지가 비쳐 힌트가 샌다. 커버 이미지도 정답 쪽에 포함.
...
## 22.10-c 만료 리마인더 + 결제-연장 감시 + 챕터 딥링크 (v2.4.3)

**만료 리마인더** (Cron `0 1 * * *` = KST 10:00, worker `runExpiryReminders`):
- 학생·챕터별 **최신** `expires_at` 기준 **D-3 / D-2 / D-1 / 당일 / D+3** 문자 (재설정 워크플로 직행 — 문구 원문 유지)
- 톤: 학습 완료 안부 → 연장(재등록) 안내 → D-3엔 "**미리 연장해도 누적이라 손해 없음**" → 챕터 딥링크
- 연장하면 만료일이 미래로 이동 → **자동 발송 중단** (해제 로직 불필요). 회당 상한 100건, 발송 시 관리자 요약 SMS
- 수동/드라이런: `POST /admin/expiry-reminders/run {dry}` (teacherGate)

**결제-연장 이중 체크** (Cron `0 * * * *` 매시, `runPaymentSyncCheck`):
- 직전 ~70분의 paid 결제 중 **10분 유예 후에도** 열람권 만료일 < 결제시점+20일이면(=C1 미반영) 관리자 ⚠️ 문자
- 결제당 약 1회 검사(70분 창) — 중복 알림 없음. 수동 전수: `POST /admin/payment-sync/check {hours, dry}`
- 배경: **결제의 실제 연장 주체는 Airtable Automation C1** (worker는 결제 기록만 생성, §5). 포인트/관리자 지급은 worker 직접(즉시)
- 한계: 자동화가 꺼져도 "결제가 없으면" 조용 → CRM 주간 루틴에 C1 눈 확인 항목 병행

**챕터 딥링크**: 학생앱 `?chapter=ID` — 로그인·홈 로드 후 해당 챕터 자동 진입 (`state.pendingChapter`, 1회성,
홈 렌더 뒤 실행이라 부팅 규칙 비침해). 리마인더·캠페인·QR 공용.

## 22.10 수학 문제은행 — math-bank (v2.4.1)

**핵심**: §22.8 파일 기반 수학 워크플로를 **DB 기반 문제은행**으로 승격. 단어풀(§22.9)과 같은 구조 —
로컬 SQLite 원천 + 웹 관리콘솔. **단, 서빙은 단어풀과 달리 기존 방식**(합본을 op_items에 저장, `@@` 참조 아님 — 이전 결정 유지).

- 📁 **`C:\Users\memoryking\00_DEV\math-bank`** (형제 폴더 — 문제·해설은 유료 자산이라 이 공개 저장소 밑 금지)
- 문제 1개 = `question_md`·`explain_md`(원천) + `question_html`·`explain_html`(학생용 재저작) + SVG assets
  — HTML은 MD의 기계 변환이 아니라 **편집 재저작**이라 둘 다 저장
- 마커 규칙은 §22.8 그대로 (`<!-- 암기카드 -->` 문제만 / `<!-- 내용 -->` 뒤에 제목·주제·풀이)
- **관리콘솔** `관리콘솔.bat` (port 8801): 목록·필터·미리보기(학생용 렌더)·편집·상태 관리
  · **📦 합본 생성** → `out/<단원>/*_합본.html` → 교사앱 "+ 📄 HTML 일괄 추가" (기존 §22.8 경로 그대로)
  · **📤 내보내기** (전체/부분, MD만/전부) → `export/` — md 첫 줄 `<!-- math-bank: id=N -->` 마커로 **재수입 시 갱신(왕복)**
- CLI: `python scripts/bank.py ingest|list|build|export`
- AI 선별: DB가 구조화되어 있어 "4점 어려운 문제 5개로 콘텐츠" 류 요청은 클로드가 SQL 선별 → build → 업로드

> 상세: `math-bank/README.md` · 작성 규칙: `11_math/수학문제_제작_가이드.md`
