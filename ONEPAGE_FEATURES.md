...
| 버튼 | 동작 | 저장 |
|---|---|---|
| **패스** | 이번 회 안 봄 → 사라지고 다음 단어가 위로 | localStorage (챕터 단위, 영구) |
| **공부** | 펼쳐서 내용 확인 | — |
| **완료** | 학습완료 ● 표시 → 자동 아래로 정렬 | DB (`op_understood`) |
| **다시** | (완료된 단어에만) 다시 미학습으로 → 위로 | DB |
| **↺** | 패스한 단어 모두 복원 | localStorage 초기화 |

### 자동 모드 전환
- 미학습 단어가 남아있을 때 → 3번째 버튼은 **완료**
- 모든 단어가 완료된 후엔 → 3번째 버튼이 **다시**
- 첫 학습 대상이 미학습일 때만 🟠 주황 강조
...
```
- 대목차 헤더(버튼들)의 화면 Y 위치 캡쳐
- 렌더 후 위치 변화량만큼 scrollBy로 보정
- 모든 학습 버튼이 이 함수로 감싸짐

→ 버튼 위치가 항상 같은 자리에 있어 연속 클릭 가능
